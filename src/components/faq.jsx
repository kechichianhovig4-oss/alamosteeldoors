  ...
  return (
    <>
      ...
      <div className="dark min-h-screen">
        <div className="bg-background-light dark:bg-background-dark font-display text-white antialiased selection:bg-primary selection:text-background-dark overflow-x-hidden">
  ...
  <div className="flex flex-1 justify-end gap-4 sm:gap-8 items-center">
    ...
    >
      <span className="truncate">Get a Quote</span>
                ...
                <div className="layout-content-container flex flex-col max-w-[960px] flex-1 w-full">
                  <div className="@container">
                    <div className="@[480px]:p-4">
                      <div 
                        className="relative flex min-h-[350px] sm:min-h-[400px] flex-col gap-4 sm:gap-6 overflow-hidden bg-cover bg-center bg-no-repeat @[480px]:gap-8 @[480px]:rounded-xl items-start justify-center px-4 pb-6 sm:pb-10 @[480px]:px-8 lg:px-10 shadow-2xl shadow-black/50 animate-fade-in-up"
                        style={{
                      ...
                      >
                        <div className="flex flex-col gap-3 sm:gap-4 text-left max-w-2xl relative z-10">
                          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-primary/10 border border-primary/20 w-fit">
                            ...
                            <span className="text-primary text-xs font-bold uppercase tracking-wider">Help Center</span>
                          </div>
                          <h1 className="text-white text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-black leading-tight tracking-[-0.033em]">
                            Frequently Asked Questions
                          </h1>
                          <h2 className="text-text-secondary text-sm sm:text-base font-normal leading-relaxed max-w-lg">
            ...
            {/* Content Area */}
            <main className="layout-container flex flex-col py-6 sm:py-8 pb-16 sm:pb-20">
                  ...
                  <div className="layout-content-container flex flex-col max-w-[960px] flex-1">
                    ...
                    <div className="flex flex-col p-4 gap-3 sm:gap-4">
                      {section.items.map((item, itemIndex) => (
                        <div 
                          key={item.id} 
                          className={`flex flex-col rounded-xl sm:rounded-2xl border transition-all duration-300 bg-surface-dark px-4 sm:px-6 py-2 ${
                            openItems[item.id] 
                        ...
                        >
                          ...
                          >
                            <p className={`text-white text-sm sm:text-base font-medium leading-normal transition-colors ${
                              openItems[item.id] ? 'text-primary' : 'group-hover:text-primary'
                            }`}>
                              {item.question}
                            </p>
                            <div className={`flex items-center justify-center w-7 h-7 sm:w-8 sm:h-8 rounded-full transition-all duration-300 ${
                              openItems[item.id] 
                                ? 'bg-primary text-background-dark rotate-180' 
                                : 'bg-[#112218] text-text-secondary'
                            }`}>
              ...
              <div className="px-4 sm:px-6 lg:px-10 xl:px-20 flex flex-1 justify-center mt-8 sm:mt-12">
                <div className="layout-content-container flex flex-col max-w-[960px] flex-1">
                  <div className="rounded-xl sm:rounded-2xl lg:rounded-3xl bg-surface-dark border border-surface-border p-6 sm:p-8 md:p-12 text-center relative overflow-hidden group">
                    {/* Decorative gradient blob */}
                    <div className="absolute top-0 left-1/2 -translate-x-1/2 w-2/3 h-full bg-primary/5 blur-[60px] sm:blur-[80px] md:blur-[100px] rounded-full pointer-events-none"></div>
                    <div className="relative z-10 flex flex-col items-center gap-4 sm:gap-6">
                      <div className="p-2 sm:p-3 bg-[#112218] rounded-full border border-surface-border shadow-lg">
                        <span className="material-symbols-outlined text-primary text-2xl sm:text-3xl">support_agent</span>
                      </div>
                      <div>
                      ...
                      <button 
                        onClick={openModal}
                        className="flex min-w-[140px] sm:min-w-[160px] cursor-pointer items-center justify-center overflow-hidden rounded-full h-11 sm:h-12 px-6 sm:px-8 bg-primary hover:bg-[#20d869] text-background-dark text-sm sm:text-base font-bold transition-all transform hover:scale-105 shadow-lg shadow-primary/20"
                      >
                        Contact Support
                      </button>
                    </div>
