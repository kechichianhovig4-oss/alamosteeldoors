...
const About = () => {
  ...
  // Employee data with correct Imgur direct image URLs
  const employees = [
    {
      id: 1,
  name: "Isaac Anthony Williams",
  title: "Number One Sales Associate for 2025",
  description: "Your trusted advisor for finding the perfect steel door. Whether you're building new or renovating, he ensures a seamless experience from initial consultation through delivery.",
  image: "https://i.imgur.com/xDHxk08.png",
  department: "Sales",
  years: "Employed since 2020",
  phone: "(726) 210-5713"
    },
    {
      id: 2,
      ...
      description: "Master metalsmith with 15+ years of experience in custom steel fabrication. Specializes in intricate scrollwork and thermal break technology.",
      image: "https://i.imgur.com/A4fsHnh.png", // Using a placeholder since album link doesn't provide specific image
    ...
    },
    {
      ...
      description: "Oversees all on-site installations with precision. Known for meticulous attention to detail and ensuring perfect weather-tight fits.",
      image: "https://i.imgur.com/182k5yV.png", // Using same placeholder
    ...
    },
    {
      id: 4,
      name: "Ken Johnson",
      title: "Design Consultant & Project Coordinator",
      description: "Bridges the gap between client vision and technical execution. Expert in translating architectural concepts into functional steel door designs.",
      image: "https://i.imgur.com/M2ddM1Y.png", // Changed to direct image URL
      department: "Design",
      years: "8 years"
    }
  ];
  ...
  return (
    <div className="min-h-screen bg-[#112218] font-display text-white">
      {/* Font imports and custom styles remain the same */}
      ...
      <div className="relative flex min-h-screen w-full flex-col bg-[#112218]">
        ...
        {/* Section Header: Legacy (unchanged) */}
        ...
        <div className="flex flex-col items-center pb-12">
          <div className="flex flex-col w-full max-w-[960px] px-4 sm:px-6 lg:px-10 xl:px-20">
            <div className="grid grid-cols-[32px_1fr] sm:grid-cols-[40px_1fr] gap-x-4 sm:gap-x-6 px-4">
              {/* Timeline items remain exactly the same */}
        ...
        {/* UPDATED: Employees Section with Square Layout */}
       <div className="relative w-full py-12 sm:py-16 md:py-20 bg-[#1c2e24]">
  <div className="flex flex-col items-center">
    <div className="flex flex-col w-full max-w-[1100px] px-4 sm:px-6 lg:px-10">
      <div className="flex flex-col gap-2 text-center mb-8 sm:mb-12">
        <h2 className="text-white text-2xl sm:text-3xl md:text-4xl font-bold">Meet Our Team</h2>
        <p className="text-gray-400 text-sm sm:text-base">The skilled professionals behind every Alamo Steel Door</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8">
        {employees.map((employee) => (
          <div 
            key={employee.id} 
            className="group relative bg-[#112218] rounded-xl sm:rounded-2xl overflow-hidden border border-white/5 hover:border-[#2bee79]/50 transition-all duration-300 hover:shadow-[0_0_25px_rgba(43,238,121,0.15)] flex flex-col"
          >
            {/* Square Employee Image Container */}
            <div className="aspect-square overflow-hidden relative">
              ...
              <div className="absolute top-3 right-3">
                <span className="bg-[#2bee79] text-[#112218] text-xs font-bold px-3 py-1.5 rounded-full uppercase tracking-wider shadow-lg">
                  {employee.department}
                </span>
              </div>
              
              {/* Employee Name Overlay - Bottom */}
              <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/90 to-transparent p-4 transform translate-y-2 group-hover:translate-y-0 transition-transform duration-300">
                <h3 className="text-white text-lg font-bold truncate">{employee.name.split(' ')[0]}</h3>
                <p className="text-[#2bee79] text-sm truncate">{employee.name.split(' ').slice(1).join(' ')}</p>
              </div>
            </div>
            ...
            <div className="p-4 sm:p-6 flex-1 flex flex-col">
              <div className="mb-3">
                <h4 className="text-white text-base sm:text-lg font-semibold">{employee.title}</h4>
                <p className="text-gray-400 text-xs sm:text-sm mt-1">{employee.description}</p>
              </div>
              ...
              <div className="mt-auto pt-4 border-t border-white/10">
                ...
                <div className="flex justify-between items-center mb-2">
                  <div className="flex items-center gap-2">
                    <span className="material-symbols-outlined text-[#2bee79] text-sm">schedule</span>
                    <span className="text-gray-400 text-xs">{employee.years}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="material-symbols-outlined text-[#2bee79] text-sm">badge</span>
                    <span className="text-gray-400 text-xs">#{employee.id.toString().padStart(3, '0')}</span>
                  </div>
                </div>
                
                {/* Phone Number - Only shows for employees with phone property */}
                {employee.phone && (
                  <div className="flex items-center gap-2 pt-2 border-t border-white/5">
                    <span className="material-symbols-outlined text-[#2bee79] text-sm">call</span>
                    <a 
                      href={`tel:${employee.phone.replace(/[^\d]/g, '')}`}
                      className="text-gray-400 text-xs hover:text-[#2bee79] transition-colors"
              ...
              <div className="mt-12 sm:mt-16 text-center max-w-2xl mx-auto">
                <p className="text-gray-300 leading-relaxed text-sm sm:text-base">
                  Our team combines decades of metallurgy expertise with modern design innovation. 
                  Each member plays a crucial role in transforming raw steel into functional works 
                  of art that protect and enhance homes across the nation.
                </p>
              </div>
            </div>
          </div>
